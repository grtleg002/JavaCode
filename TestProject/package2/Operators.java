...
public class Operators {
	...
	public static void main(String[] args) {
		...
		//연산식에 증감연산과 다른 연산(대입연산)이 같이 있는경우
		int x1 = 10;
		int y1 = 0;
		
		y1 = x1++; // ++가 대입연산 이후로 밀림. x1을 먼저 y1에 대입(10) 시키고 x1을 1 증가.
		System.out.println(y1);
		y1 = ++x1; // 11인 x1을 먼저 증가 시키고(12가 됨) y1에 대입 -> y1은 12가 대입
		System.out.println(y1);
		...
		System.out.println(y+++10 ); //y를 10 더하고 난 후에 y값을 1 증가시킴
		System.out.println(y); // 위 라인에서 증가된 y가 출력 > 2
		
		// 이항 연산자 : + - * / %
		// 주의: 연산 결과 어버플로우 발생 가능 : 연산 결과값이 너무 커서 제대로 저장이 일어나지 않는 경우
		long x2 = 1000000; //백만
		long y2 = 1000000; //백만
		
		//int z2 = x2 *y2; // 조 단위가 됨 -> x2* y2의 결과가 int형 반환(오버플로우 발생)
		long z2 = x2 *y2; 
		...
		//비교 연산자 : == != -> boolean type 변환
		System.out.println(x2==y2);
		System.out.println(x2 != y2);
		
		//문자열 비교 - 문자열 변수는 참조변수 이므로 참조값 (주소)을 저장하고 있음
		...
		String name1 ="홍길동";
		String name2 ="홍길동";
		//name1과 name2에 동일한 값이 저장되어 있는지 비교연산
		
		//참조 주소 비교
		System.out.println(name1 == name2);
		System.out.println(name1 != name2);
		
		//문자열 객체 메소드 equals를 활용, 값을 비교하는 기능임.
		System.out.println(name1.equals(name2));
		
		//논리 연산자 : && || ! ^ 연산 가능 -> true false 값 반환
		...
		// 비트 산술 연산자 : a<<b :a값을 비트표현하고 왼쪽으로 b 만큼 이동 , a>>b :a값을 비트표현하고 오른쪽으로 b 만큼 이동
